...
You can find a platformio.ini template you can use when creating a project for a MegaCore compatible device below. The most common functionality is available in this template. More information on what each line means can be found futher down on this page.
...
Recommended baud rate for the particular clock speed is `emphasized`.
...
**Note that every flag has to be on its own line, and they have to be indented:**
```ini
upload_flags = -PUSB
  -B32
  -v
